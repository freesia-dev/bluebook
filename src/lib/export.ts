...
export const exportAllTables = async () => {
  ...
  try {
    ...
    const [suratMasukData, suratKeluarData, sppkData, pkData, kkmpakData] = await Promise.all([
      getSuratMasuk(),
      getSuratKeluar(),
      getSPPK(),
      getPK(),
    ...
    // Surat Masuk
    const suratMasuk = suratMasukData.map(item => ({
      'No': item.nomor,
    ...
    // Surat Keluar
    const suratKeluar = suratKeluarData.map(item => ({
      'No': item.nomor,
      'Nomor Agenda': item.nomorAgenda,
      'Kode Surat': item.kodeSurat,
    ...
    // SPPK Telihan
    const sppkTelihan = sppkData.filter(s => s.type === 'telihan').map(item => ({
      'No': item.nomor,
    ...
    // SPPK Meranti
    const sppkMeranti = sppkData.filter(s => s.type === 'meranti').map(item => ({
      'No': item.nomor,
      'Nomor SPPK': item.nomorSPPK,
      'Nama Debitur': item.namaDebitur,
      'Jenis Kredit': item.jenisKredit,
    ...
    // PK Telihan
    const pkTelihan = pkData.filter(s => s.type === 'telihan').map(item => ({
      'No': item.nomor,
    ...
    // PK Meranti
    const pkMeranti = pkData.filter(s => s.type === 'meranti').map(item => ({
      'No': item.nomor,
      'Nomor PK': item.nomorPK,
      'Nama Debitur': item.namaDebitur,
      'Jenis Kredit': item.jenisKredit,
    ...
    // KK & MPAK Telihan
    const kkmpakTelihan = kkmpakData.filter(s => s.type === 'telihan').map(item => ({
      'No': item.nomor,
    ...
    // Agenda & MPAK Meranti
    const kkmpakMeranti = kkmpakData.filter(s => s.type === 'meranti').map(item => ({
      'No': item.nomor,
